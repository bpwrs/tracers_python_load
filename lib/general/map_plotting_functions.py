...
def retrieve_ead_data(year,month,day,spacecraft,local_dir=None):
    ...
    base_url = f'https://tracers-portal.physics.uiowa.edu/teams/flight/SOC/TS{spacecraft}/ead/{filetype}'
    date_url = f'{base_url}/'
...
def read_ead_files(files2load, start=None, end=None, params=None, quiet=None):
    ...
    for elem in out_dict['UTC']:
        out_dict['DT'].append(dt.datetime.fromtimestamp(elem,dt.timezone.utc))
...
def geographic_plot(t0, tf, sc, ax=None, nightshade=None, hemisphere=None, cmap=None, orbit_path=None):
    ...
    hemisphere = Default is north; valid input is 'north' or 'south'
    ...
    if orbit_path is None or orbit_path is True:
        cm = ax.scatter(ead_data[f'ts{sc}_ead_lon_geo'],ead_data[f'ts{sc}_ead_lat_geo'],c=ead_data['UTC'],\
                        cmap=cmap,zorder=2,marker='.',s=3, transform=ccrs.PlateCarree())
